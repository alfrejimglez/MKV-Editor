...
class EditorMKV(QWidget):
    ...
    def convertToMP4(self):
        if self.output_file:
            ...
            has_pgs = False
            try:
                mkvmerge_output = subprocess.check_output(["mkvmerge", "-i", "-F", "json", edited_mkv]).decode("utf-8")
                track_info = json.loads(mkvmerge_output)
                for track in track_info["tracks"]:
            ...
            if has_pgs:
                print("Detectados subtítulos PGS. Haciendo OCR...")
                temp_sup = os.path.splitext(edited_mkv)[0] + "_pgs.sup"
                temp_srt = os.path.splitext(edited_mkv)[0] + "_pgs.srt"

                try:
                    # Extraer PGS a SUP
                    subprocess.run(["mkvextract", "tracks", edited_mkv, f"{pgs_track_id}:{temp_sup}"], check=True)
                    print(f"Extraído PGS a SUP: {temp_sup}")
                    ...
                    print(f"Convertido a SRT con OCR: {temp_srt}")

                    # FFmpeg con SRT incrustado
                    args = ["ffmpeg", "-y", "-i", edited_mkv, "-i", temp_srt, "-map", "0:v", "-map", "0:a", "-map", "1:0", "-c:v", "copy", "-c:a", "aac", "-b:a", "128k", "-ac", "2", "-c:s", "mov_text", "-metadata:s:a:0", "language=spa", "-metadata:s:s:0", "language=spa", output_file]
                    subprocess.run(args, check=True)
                    print(f"Convertido a MP4 con subtítulos: '{output_file}'")

                except Exception as e:
                    print(f"Error en OCR, intentando sin subtítulos: {e}")
                    # Fallback: convertir sin subtítulos
                    args = ["ffmpeg", "-y", "-i", edited_mkv, "-c:v", "copy", "-c:a", "aac", "-b:a", "128k", "-ac", "2", "-metadata:s:a:0", "language=spa", output_file]
                    subprocess.run(args, check=True)
                    print(f"Convertido a MP4 sin subtítulos: '{output_file}'")
            else:
                # Sin PGS: conversión simple
                args = ["ffmpeg", "-y", "-i", edited_mkv, "-c:v", "copy", "-c:a", "aac", "-b:a", "128k", "-ac", "2", "-c:s", "mov_text", "-metadata:s:a:0", "language=spa", output_file]
                subprocess.run(args, check=True)
            ...
            try:
                ...
                pgs_srt_files = glob.glob(os.path.splitext(edited_mkv)[0] + "_pgs*")
                for srt_file in pgs_srt_files:
                    try:
                        if os.path.exists(srt_file):
                            os.remove(srt_file)
                            print(f"Temporal eliminado: {srt_file}")
                    except Exception:
                        pass
