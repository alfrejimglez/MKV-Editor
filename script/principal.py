...
def resource_path(relative_path):
    """Obtiene la ruta correcta para los recursos empaquetados con PyInstaller."""
    try:
        # Si el script está empaquetado en un archivo .exe
        if getattr(sys, 'frozen', False):
            # La variable sys._MEIPASS es donde PyInstaller coloca los archivos
            base_path = sys._MEIPASS
        else:
            # Si el script no está empaquetado, usa la ruta actual
...
class EditorMKV(QWidget):
    ...
    def playSound(self):
        pygame.mixer.init()
        sound_path = resource_path("finish.wav")  # Usar el método resource_path para obtener la ruta correcta
