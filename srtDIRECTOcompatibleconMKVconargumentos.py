...
def main():
    ...
    # Si recibe un archivo como argumento, procesar solo ese
    if len(sys.argv) > 1:
        ...
        if not srt and not has_internal_subs(ffprobe, file_arg):
            print(f"[SKIP] {file_arg.name}")
            return
        # ⚠️ CASO DURO: 1 srt pero no match claro → pedir confirmación
        if srt and not auto_ok:
            print("\n⚠️ POSIBLE MATCH MANUAL")
            print(f"🎬 Vídeo: {file_arg.name}")
            print(f"📝 Sub:   {srt.name}")
            resp = input("¿Quieres quemarlo? [y/n]: ").strip().lower()
            if resp != "y":
                print("[SKIP MANUAL]")
                return
        subtitle = srt if srt else file_arg
        ok = 0
        fail = 0
        if process_file(ffmpeg, ffprobe, file_arg, subtitle, style):
            ok += 1
        else:
            fail += 1
        print("\nResumen:")
    ...
    # Si no hay argumento, modo clásico (todos los mp4/mkv)
    style = input("Estilo [B]lanco, [R]ojo, [G]ris: ").strip().lower()
    style = (
        "rojo" if style in ["r", "rojo"]
        else "gris" if style in ["g", "gris"]
